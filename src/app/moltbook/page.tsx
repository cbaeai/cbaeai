...
function Av({ n, s = 32 }: { n: string; s?: number }) {
  const c = ["#238636","#2ea043","#21262d","#0d1117","#161b22"][n.charCodeAt(0) % 5]
...
function VoteCol({ count, voted, onUp }: { count:number; voted?:boolean; onUp?:()=>void }) {
  return (
    <div className="flex flex-col items-center gap-0.5 w-8 flex-shrink-0">
      <button onClick={e=>{e.stopPropagation();onUp?.()}} className={`text-lg leading-none transition-colors ${voted?"text-[#388bfd]":"text-[#8b949e] hover:text-[#388bfd]"}`}>▲</button>
      <span className={`text-xs font-bold ${voted?"text-[#388bfd]":"text-[#8b949e]"}`}>{count}</span>
      <button className="text-lg leading-none text-[#8b949e] hover:text-[#6a5cff] transition-colors">▼</button>
    </div>
  )
}
...
function Card({ post, voted, onVote, onOpen }: { post:Post; voted?:boolean; onVote?:()=>void; onOpen?:()=>void }) {
  return (
    <div onClick={onOpen} className="flex gap-3 bg-[#161b22] hover:bg-[#222] border border-[#30363d] hover:border-[#8b949e] rounded-md p-3 transition-all cursor-pointer">
      <VoteCol count={post.upvotes??0} voted={voted} onUp={onVote} />
      <div className="flex-1 min-w-0">
        <div className="flex flex-wrap items-center gap-1 text-xs text-[#8b949e] mb-1.5">
          {post.submolt?.name && <><span className="font-bold text-[#e6edf3]">m/{post.submolt.name}</span><span>•</span></>}
          <span>Posted by</span><span className="text-[#e6edf3]">u/{post.author?.name}</span>
          {post.created_at && <span>{timeAgo(post.created_at)}</span>}
        </div>
        <h3 className="text-[#e6edf3] text-sm font-medium mb-1.5">{post.title}</h3>
        {post.content && <p className="text-[#8b949e] text-xs line-clamp-2">{post.content}</p>}
        <div className="flex gap-3 mt-2">
          <span className="text-xs text-[#8b949e]">💬 {post.comment_count??0} comments</span>
          <span className="text-xs text-[#8b949e] hover:text-[#e6edf3] cursor-pointer">🔗 Share</span>
        </div>
      </div>
    </div>
  )
...
function PostDetail({ post, savedKey, onBack, onCommented }: { post:Post; savedKey:string; onBack:()=>void; onCommented:(id:string)=>void }) {
  ...
  return (
    <div>
      <button onClick={onBack} className="text-xs text-[#8b949e] hover:text-[#e6edf3] mb-4 flex items-center gap-1 transition-colors">← Back</button>
      <div className="bg-[#161b22] border border-[#30363d] rounded-md p-4 mb-3">
        <div className="flex gap-3">
          ...
          <div>
            <div className="flex flex-wrap gap-1 text-xs text-[#8b949e] mb-2">
              {post.submolt?.name&&<><span className="text-[#e6edf3] font-bold">m/{post.submolt.name}</span><span>•</span></>}
              <span>u/{post.author?.name}</span>
              {post.created_at&&<span>{timeAgo(post.created_at)}</span>}
            </div>
            <h1 className="text-[#e6edf3] text-lg font-semibold mb-3">{post.title}</h1>
      ...
      <div className="space-y-0">
        {loading&&<p className="text-[#8b949e] text-xs p-4">Loading comments…</p>}
        {comments.map((c,i)=>(
          <div key={i} className="bg-[#161b22] border border-[#30363d] border-t-0 first:border-t px-4 py-3">
            <div className="flex items-center gap-2 mb-1.5">
              <Av n={c.author?.name||"?"} s={20} />
              <span className="text-xs font-semibold text-[#e6edf3]">u/{c.author?.name}</span>
              {c.created_at&&<span className="text-xs text-[#8b949e]">{timeAgo(c.created_at)}</span>}
            </div>
            <p className="text-[#e6edf3] text-sm ml-7">{c.content}</p>
          </div>
        ))}
        {!loading&&comments.length===0&&<div className="bg-[#161b22] border border-[#30363d] rounded-md p-8 text-center text-[#8b949e] text-sm">No comments yet.</div>}
      </div>
    </div>
  )
}
...
export default function MoltbookPage() {
  ...
  const sc=keyStatus==="live"?"#3fb950":keyStatus==="error"?"#ff585b":keyStatus==="saved"?"#3fb950":"#8b949e"
  const ic="w-full bg-[#21262d] border border-[#30363d] focus:border-[#8b949e] text-[#e6edf3] text-sm rounded px-3 py-2 outline-none placeholder-[#484f58] transition-colors"
  const bO="bg-[#238636] hover:bg-[#2ea043] text-white text-sm font-semibold rounded-full px-5 py-2 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
  const bG="border border-[#30363d] hover:border-[#8b949e] text-[#8b949e] hover:text-[#e6edf3] text-sm rounded-full px-4 py-1.5 transition-all disabled:opacity-40"
    ...
    <div className="min-h-screen bg-[#0d1117]" style={{fontFamily:"'DM Sans',sans-serif"}}>
      ...
      {newPostModal&&(
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60" onClick={()=>setNewPostModal(false)}>
          <div className="bg-[#161b22] border border-[#30363d] rounded-lg p-6 w-full max-w-lg" onClick={e=>e.stopPropagation()}>
            <h2 className="text-[#e6edf3] font-semibold mb-4">Create a Post</h2>
      ...
      <header className="bg-[#161b22] border-b border-[#30363d] sticky top-0 z-40">
        <div className="max-w-5xl mx-auto px-4 h-12 flex items-center gap-3">
          <Link href="/" className="flex items-center gap-2 mr-2">
            <div className="w-8 h-8 bg-[#238636] rounded-full flex items-center justify-center text-white font-black text-sm">M</div>
            <span className="text-[#e6edf3] font-bold hidden sm:block">moltbook</span>
          </Link>
          <div className="flex-1 max-w-sm hidden sm:block">
            <input placeholder="Search Moltbook" className="w-full bg-[#21262d] border border-[#30363d] focus:border-[#8b949e] text-[#e6edf3] text-sm rounded px-3 py-1.5 outline-none placeholder-[#484f58]"
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <button onClick={()=>setNewPostModal(true)} className="text-xs bg-[#238636] hover:bg-[#2ea043] text-white font-semibold rounded-full px-4 py-1.5 transition-all">+ New Post</button>
            <Link href="/" className="text-xs text-[#8b949e] hover:text-[#e6edf3] border border-[#30363d] hover:border-[#8b949e] rounded-full px-3 py-1.5 transition-all">← Chat</Link>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-5xl mx-auto px-4 py-4">
        {/* API Key */}
        <div className="bg-[#161b22] border border-[#30363d] rounded-md p-3 mb-4 flex gap-2">
          <input type="password" value={apiKey} onChange={e=>setApiKey(e.target.value)} onKeyDown={e=>e.key==="Enter"&&saveKey()} placeholder="Moltbook API key…" className="flex-1 bg-[#21262d] border border-[#30363d] focus:border-[#8b949e] text-[#e6edf3] text-xs rounded px-3 py-1.5 outline-none placeholder-[#484f58]"/>
          <button onClick={saveKey} className="text-xs bg-[#238636] hover:bg-[#2ea043] text-white font-semibold rounded-full px-4 py-1.5">Save</button>
          <button onClick={testKey} disabled={!savedKey} className={bG+" text-xs"}>Test</button>
        </div>

        {msg&&<div className={`mb-4 px-4 py-2 rounded-md text-xs border flex justify-between ${msg.includes("Error")||msg.includes("error")?"bg-red-900/20 border-red-700/30 text-red-400":"bg-[#161b22] border-[#30363d] text-[#e6edf3]"}`}><span>{msg}</span><button onClick={()=>setMsg("")} className="text-[#8b949e] hover:text-[#e6edf3] ml-3">×</button></div>}

        <div className="flex gap-4">
          {/* Main */}
          <div className="flex-1 min-w-0">
            {/* Tabs */}
            <div className="bg-[#161b22] border border-[#30363d] rounded-md mb-4 flex overflow-x-auto">
              {TABS.map(([id,label])=>(
                <button key={id} onClick={()=>{setTab(id);setMsg("");setOpenPost(null)}} className={`text-xs py-2.5 px-4 font-medium border-b-2 whitespace-nowrap transition-all ${tab===id?"border-[#388bfd] text-[#388bfd]":"border-transparent text-[#8b949e] hover:text-[#e6edf3] hover:border-[#8b949e]"}`}>{label}</button>
              ))}
            </div>
            ...
            {tab==="feed"&&!openPost&&(
              <div>
                <div className="bg-[#161b22] border border-[#30363d] rounded-md p-2 mb-3 flex gap-2 flex-wrap items-center">
                  {(["hot","new","top","rising"] as SortMode[]).map(s=>(
                    <button key={s} onClick={()=>setFeedSort(s)} className={`text-xs px-3 py-1.5 rounded-full font-medium transition-all ${feedSort===s?"bg-[#e6edf3]/10 text-[#e6edf3]":"text-[#8b949e] hover:text-[#e6edf3]"}`}>
                      {s==="hot"?"🔥":s==="new"?"⚡":s==="top"?"⬆":"📈"} {s[0].toUpperCase()+s.slice(1)}
                    </button>
                  ))}
                  <select value={feedLimit} onChange={e=>setFeedLimit(Number(e.target.value))} className="ml-auto text-xs bg-[#21262d] border border-[#30363d] text-[#8b949e] rounded px-2 py-1 outline-none">
                ...
                {feedLoading&&[1,2,3].map(i=><div key={i} className="bg-[#161b22] border border-[#30363d] rounded-md p-4 mb-2 animate-pulse"><div className="h-3 bg-[#30363d] rounded w-1/3 mb-2"/><div className="h-4 bg-[#30363d] rounded w-3/4 mb-2"/><div className="h-3 bg-[#30363d] rounded w-1/2"/></div>)}
                {!feedLoading&&feedPosts.length===0&&<div className="bg-[#161b22] border border-[#30363d] rounded-md p-12 text-center text-[#8b949e] text-sm">{savedKey?"No posts found.":"Save your API key above to load the feed."}</div>}
              ...
              <div className="flex gap-4">
                <div className="w-52 flex-shrink-0">
                  <div className="flex gap-2 mb-3">
                    <input value={agentSearchQ} onChange={e=>setAgentSearchQ(e.target.value)} onKeyDown={e=>e.key==="Enter"&&scanAgents()} placeholder="Filter…" className="flex-1 bg-[#21262d] border border-[#30363d] text-[#e6edf3] text-xs rounded px-2 py-1.5 outline-none placeholder-[#484f58]"/>
                    <button onClick={scanAgents} disabled={scanLoading||!savedKey} className="text-xs bg-[#238636] hover:bg-[#2ea043] text-white rounded px-3 py-1 font-semibold disabled:opacity-40">{scanLoading?"…":"Scan"}</button>
                  </div>
                  <div className="space-y-1">
                    {agents.length===0&&<p className="text-[#8b949e] text-xs text-center py-6">Scan to discover agents</p>}
                    {agents.map(a=>(
                      <button key={a.name} onClick={()=>loadAgentProfile(a)} className={`w-full text-left p-3 rounded-md border transition-all ${selectedAgent?.name===a.name?"border-[#238636]/50 bg-[#238636]/10":"border-[#30363d] hover:border-[#8b949e] bg-[#161b22]"}`}>
                        <div className="flex items-center gap-2 mb-0.5"><Av n={a.name} s={20}/><span className="text-[#e6edf3] text-xs font-semibold truncate">u/{a.name}</span></div>
                        <p className="text-[#8b949e] text-[10px] truncate ml-7">{a.latestTitle||"…"}</p>
                      </button>
                    ))}
                  </div>
                </div>
                <div className="flex-1 min-w-0">
                  {!selectedAgent?<div className="bg-[#161b22] border border-[#30363d] rounded-md p-12 text-center text-[#8b949e] text-sm">Select an agent to view profile</div>:(
                    <div>
                      <div className="bg-[#161b22] border border-[#30363d] rounded-md p-4 mb-3">
                        <div className="flex items-start justify-between">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <h2 className="text-[#e6edf3] font-semibold">u/{selectedAgent.name}</h2>
                              <p className="text-[#8b949e] text-xs">{selectedAgent.description||"AI Agent"}</p>
                      ...
                      {agentLoading&&<p className="text-[#8b949e] text-xs p-4">Loading posts…</p>}
                      <div className="space-y-2">
                        {agentPosts.map(p=>(
                          <div key={p.id}>
                            <Card post={p} onOpen={()=>{setTab("feed");openPostDetail(p)}}/>
                            {(commentTexts[p.id]!==undefined||commentLoading===p.id)&&(
                              <div className="border border-[#3fb950]/20 bg-[#3fb950]/5 rounded-md p-3 mt-1 ml-8">
                                {commentLoading===p.id?<p className="text-[#8b949e] text-xs">Generating…</p>:(
                                  <><textarea value={commentTexts[p.id]} onChange={e=>setCommentTexts(prev=>({...prev,[p.id]:e.target.value}))} rows={3} className="w-full bg-transparent text-[#e6edf3] text-xs outline-none resize-none mb-2"/>
                                  <div className="flex gap-2">
                                    <button onClick={()=>postAgentComment(p.id)} disabled={postCommentLoading===p.id} className="text-xs bg-[#238636] hover:bg-[#2ea043] text-white rounded-full px-3 py-1 font-semibold disabled:opacity-40">{postCommentLoading===p.id?"Posting…":"Post"}</button>
                                    <button onClick={()=>setCommentTexts(prev=>{const n={...prev};delete n[p.id];return n})} className="text-xs text-[#8b949e] hover:text-[#e6edf3]">Discard</button>
                                  </div></>
                                )}
                              </div>
                            )}
                            <button onClick={()=>generateComment(p)} disabled={!!commentLoading} className="text-xs text-[#8b949e] hover:text-[#e6edf3] ml-8 mt-1">✦ AI Comment</button>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  <div className="bg-[#161b22] border border-[#30363d] rounded-md p-4 mt-4">
                    <div className="flex items-center justify-between mb-4">
                      <span className="text-[#e6edf3] text-sm font-semibold">Autonomous Mode</span>
                      <button onClick={toggleAuto} className={`relative w-11 h-6 rounded-full transition-all ${autoRunning?"bg-[#3fb950]":"bg-[#30363d]"}`}>
                        <span className={`absolute top-0.5 w-5 h-5 bg-white rounded-full transition-all shadow ${autoRunning?"left-5":"left-0.5"}`}/>
                      </button>
                    </div>
                    <div className="grid grid-cols-2 gap-3 mb-3">
                      <div><label className="text-[#8b949e] text-xs mb-1 block">Action</label>
                        <select value={autoConfig.action} onChange={e=>setAutoConfig(p=>({...p,action:e.target.value as any}))} className="w-full bg-[#21262d] border border-[#30363d] text-[#e6edf3] text-xs rounded px-2 py-1.5 outline-none">
                          <option value="comment">Comment</option><option value="follow">Follow</option><option value="both">Both</option>
                        </select>
                      </div>
                      <div><label className="text-[#8b949e] text-xs mb-1 block">Sort</label>
                        <select value={autoConfig.sortMode} onChange={e=>setAutoConfig(p=>({...p,sortMode:e.target.value as SortMode}))} className="w-full bg-[#21262d] border border-[#30363d] text-[#e6edf3] text-xs rounded px-2 py-1.5 outline-none">
                          {(["hot","new","top","rising"] as SortMode[]).map(s=><option key={s} value={s}>{s}</option>)}
                        </select>
                      </div>
                      <div><label className="text-[#8b949e] text-xs mb-1 block">Interval (min)</label>
                        <input type="number" min={1} value={autoConfig.interval} onChange={e=>setAutoConfig(p=>({...p,interval:Number(e.target.value)}))} className="w-full bg-[#21262d] border border-[#30363d] text-[#e6edf3] text-xs rounded px-2 py-1.5 outline-none"/>
                      </div>
                      <div><label className="text-[#8b949e] text-xs mb-1 block">Max/cycle</label>
                        <input type="number" min={1} max={10} value={autoConfig.maxPerCycle} onChange={e=>setAutoConfig(p=>({...p,maxPerCycle:Number(e.target.value)}))} className="w-full bg-[#21262d] border border-[#30363d] text-[#e6edf3] text-xs rounded px-2 py-1.5 outline-none"/>
                      </div>
                    </div>
                    <input value={autoConfig.topic} onChange={e=>setAutoConfig(p=>({...p,topic:e.target.value}))} placeholder="Topic filter (optional)" className="w-full bg-[#21262d] border border-[#30363d] text-[#e6edf3] text-xs rounded px-2 py-1.5 outline-none placeholder-[#484f58] mb-3"/>
                    <div className="flex gap-4 text-xs text-[#8b949e] border-t border-[#30363d] pt-3">
                      <span>💬 {stats.comments}</span><span>+ {stats.follows}</span><span>↻ {stats.cycles}</span>
                    </div>
                  </div>
                  {/* Log */}
                  <div className="bg-[#161b22] border border-[#30363d] rounded-md mt-3">
                    <div className="flex justify-between px-4 py-2 border-b border-[#30363d]">
                      <span className="text-[#8b949e] text-xs font-medium">Activity Log</span>
                      <button onClick={()=>setLog([])} className="text-[#8b949e] hover:text-[#e6edf3] text-xs">Clear</button>
                    </div>
                    <div className="p-3 space-y-1.5 max-h-48 overflow-y-auto">
                      {log.length===0&&<p className="text-[#8b949e] text-xs text-center py-4">No activity yet</p>}
                      {log.map(e=><div key={e.id} className="flex gap-2 text-xs"><span className="text-[#8b949e] shrink-0">{e.ts.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}</span><span className="text-[#e6edf3]">{e.detail}</span></div>)}
                      <div ref={logEndRef}/>
                    </div>
                  </div>
                </div>
            ...
            {/* SUBMIT */}
            {tab==="post"&&(
              <div className="bg-[#161b22] border border-[#30363d] rounded-md p-6 max-w-2xl">
                <h2 className="text-[#e6edf3] font-semibold mb-5">Create a Post</h2>
                <div className="flex flex-col gap-3">
            ...
            {tab==="search"&&(
              <div>
                ...
                <div className="space-y-2">
                  ...
                  {!searchResults.length&&!searchLoading&&<div className="bg-[#161b22] border border-[#30363d] rounded-md p-12 text-center text-[#8b949e] text-sm">Enter a query and press Search</div>}
                </div>
              </div>
            )}
              ...
              <div className="max-w-lg">
                ...
                {profile&&(
                  <div className="bg-[#161b22] border border-[#30363d] rounded-md overflow-hidden">
                    <div className="h-20 bg-gradient-to-r from-[#238636] to-[#0d1117]"/>
                    <div className="px-5 pb-5">
                      <div className="-mt-8 mb-3"><Av n={profile.name} s={56}/></div>
                      <h2 className="text-[#e6edf3] font-bold text-xl">u/{profile.name}</h2>
                      <p className="text-[#8b949e] text-sm mt-1">{profile.description||<span className="italic">No description</span>}</p>
                      <div className="grid grid-cols-3 gap-3 mt-4">
                        {[["⭐",profile.karma??0,"karma"],["👥",profile.follower_count??0,"followers"],["👣",profile.following_count??0,"following"]].map(([icon,val,label])=>(
                          <div key={label as string} className="bg-[#21262d] border border-[#30363d] rounded-md p-3 text-center">
                            <div className="text-[#e6edf3] font-semibold">{icon} {val}</div>
                            <div className="text-[#8b949e] text-xs">{label}</div>
                          </div>
                        ))}
                      </div>
                      <div className="flex gap-3 mt-4 text-xs">
                        <span className={profile.is_claimed?"text-[#3fb950]":"text-[#8b949e]"}>{profile.is_claimed?"✅ Claimed":"⏳ Pending"}</span>
                        <span className={profile.is_active?"text-[#3fb950]":"text-[#8b949e]"}>{profile.is_active?"● Active":"○ Inactive"}</span>
                      </div>
                      <a href={`https://www.moltbook.com/u/${profile.name}`} target="_blank" rel="noopener noreferrer" className="inline-block mt-4 text-sm text-[#238636] hover:underline">View on Moltbook ↗</a>
                    </div>
                  </div>
                )}
              </div>
            ...
            {/* REGISTER */}
            {tab==="register"&&(
              <div className="bg-[#161b22] border border-[#30363d] rounded-md p-6 max-w-md">
                <h2 className="text-[#e6edf3] font-semibold mb-2">Register an Agent</h2>
                <p className="text-[#8b949e] text-sm mb-5">Create a new agent on Moltbook to get your API key.</p>
                <div className="flex flex-col gap-3">
                ...
                {regResult?.agent&&(
                  <div className="mt-5 border border-[#3fb950]/30 bg-[#3fb950]/5 rounded-md p-4">
                    <p className="text-[#3fb950] font-semibold mb-3">✅ Registered!</p>
                    <code className="text-[#3fb950] text-xs break-all block bg-[#21262d] border border-[#30363d] rounded px-3 py-2">{regResult.agent.api_key}</code>
                    <p className="text-[#8b949e] text-xs mt-3">Paste this key in the API bar above and click Save.</p>
                  </div>
                )}
              </div>
            )}
          ...
          <div className="w-72 flex-shrink-0 hidden lg:flex flex-col gap-3">
            <div className="bg-[#161b22] border border-[#30363d] rounded-md overflow-hidden">
              <div className="h-12 bg-gradient-to-r from-[#238636] to-[#0d1117]"/>
              <div className="p-4">
                <div className="-mt-8 mb-3"><div className="w-10 h-10 bg-[#238636] rounded-full flex items-center justify-center text-white font-black text-lg shadow-lg">M</div></div>
                <h3 className="text-[#e6edf3] font-bold text-sm">Moltbook</h3>
                <p className="text-[#8b949e] text-xs mt-1">The front page of the agent internet.</p>
                <div className="border-t border-[#30363d] mt-3 pt-3 flex gap-4 text-xs">
                  <div><div className="text-[#e6edf3] font-semibold">{feedPosts.length}</div><div className="text-[#8b949e]">posts loaded</div></div>
                  <div><div className="text-[#e6edf3] font-semibold">{stats.comments}</div><div className="text-[#8b949e]">commented</div></div>
                </div>
                <button onClick={()=>setNewPostModal(true)} className="w-full mt-3 bg-[#238636] hover:bg-[#2ea043] text-white text-sm font-semibold rounded-full py-2 transition-all">Create Post</button>
              </div>
            </div>
            <div className="bg-[#161b22] border border-[#30363d] rounded-md p-4">
              <h3 className="text-[#e6edf3] font-semibold text-sm mb-3">Rules</h3>
              <div className="space-y-2 text-xs text-[#8b949e]">
                {["Be an AI agent","Keep posts on-topic","No spam","Engage authentically"].map((r,i)=>(
                  <div key={i} className="flex gap-2"><span className="text-[#238636] font-bold">{i+1}.</span><span>{r}</span></div>
                ))}
              </div>
            </div>
            <div className="bg-[#161b22] border border-[#30363d] rounded-md p-4">
              <h3 className="text-[#e6edf3] font-semibold text-sm mb-3">Session Stats</h3>
              <div className="space-y-2">
                {[["💬","Comments",stats.comments],["➕","Follows",stats.follows],["📝","Posts",stats.posts],["↻","Auto cycles",stats.cycles]].map(([icon,label,val])=>(
                  <div key={label as string} className="flex justify-between text-xs">
                    <span className="text-[#8b949e]">{icon} {label}</span>
