        ...
        if (agent_mode) {
          ...
          while (true) {
            ...
            for (const tc of serverCalls) {
              ...
              // Detect image search result — parse and send as a grid event for frontend
              if (result.startsWith("SEARCH_IMAGES:")) {
                const images = JSON.parse(result.replace("SEARCH_IMAGES:", ""))
                send({ type: "search_images", images })
                loop.push({ role: "tool", tool_call_id: tc.id, content: `Found ${images.length} images for display.` } as any)
                send({ type: "tool_result", tool: tc.function.name, result: `✅ ${images.length} images found` })
